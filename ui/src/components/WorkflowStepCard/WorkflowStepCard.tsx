      ...
      <div className="poppy:bg-zinc-700 poppy:text-zinc-200 bg-s-base2 text-s-base01 dark:bg-s-base02 dark:text-s-base1 max-w-prose">
        ...
        <div className="text-sm px-5 pb-5">{props.step.info.description}</div>
          ...
          <div className="px-5 pb-5 text-sm">
            <span>contract:</span>{' '}
            <a href="https://etherscan.io/address/0x0000000000000000000000000000000000000000" rel="noreferrer" target="_blank" className="text-s-blue underline cursor-pointer">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          ...
          <div className="px-2 text-sm text-s-base1 dark:text-s-base01 poppy:text-zinc-500 flex items-center">
            <AnimatePresence>
              {active && (
                <motion.div role="status" initial={{ width: 0, marginRight: 0 }} exit={{ width: 0, marginRight: 0 }} animate={{ width: 16, marginRight: 5 }}>
                  <svg className="inline mr-2 w-3 h-3 text-transparent animate-spin fill-s-base1" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <AnimatePresence exitBeforeEnter>
              {props.lastEvent?.statusMessage && (
                <motion.div key={props.lastEvent.statusMessage} initial={{opacity: 0, y: -5}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: 5}}>
                  {props.lastEvent?.statusMessage}
                </motion.div>
              )}
            </AnimatePresence>
