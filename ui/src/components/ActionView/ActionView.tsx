...
import { Asset, AssetAmount, BLOCKCHAIN_INFO, formatMoney, getAssetInfo, getStepInfo, WorkflowActionInput } from '@fmp/sdk'

const BalanceCard = (props: { asset: Asset; amount: AssetAmount }): JSX.Element => {
  ...
  return (
    <div className="rounded-full py-2 px-6 inline-flex items-center text-s-base0 dark:text-s-base00 space-x-5">
      <div className="w-10 h-10 relative translate-y-1">
        <img className="absolute w-8 h-8 rounded-full" src={assetInfo.iconUrl} />
        <div className="absolute w-4 h-4 rounded-md right-1 bottom-1 overflow-hidden flex items-center justify-center">
          <img className="w-4 h-4" src={BLOCKCHAIN_INFO[props.asset.chain].iconUrl} />
        </div>
      </div>
      <div className="flex flex-col">
        <div className="font-bold">
          {formatMoney(props.amount, assetInfo.decimals)} {props.asset.symbol}
        </div>
        <div className="text-s-base1 dark:text-s-base01">on {props.asset.chain}</div>
      </div>
    </div>
  )
}

export const ActionView = (props: { step: WorkflowActionInput; stepIndex: number; children?: React.ReactNode }): JSX.Element => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          />
          <BalanceCard asset={props.step.inputAsset} amount={props.step.amount!} />
          ...
          <div className="rounded-full p-2 bg-s-base2 dark:bg-s-base02 text-s-base0 dark:text-s-base00 font-bold flex items-center space-x-2 transition group-hover:bg-s-base2 dark:group-hover:bg-s-base02">
            <img className="inline w-6 h-6 rounded-full" src={stepInfo.iconUrl} />
            <div className="inline">{stepInfo.name}</div>
          </div>
          <div className="border-t-4 border-dashed border-s-base2 dark:border-s-base02 flex grow shrink mx-5 group-hover:border-s-base1/25" />
