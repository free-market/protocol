...
import {
  serialize as borshSerialize,
  field,
  OverrideType,
} from "@dao-xyz/borsh";
import BN from "bn.js";

const wormholeCoreBridgeAddress =
...
describe("fmp", () => {
  ...
  it("Is initialized!", async () => {
    ...
    const wh = new WormholeTransfer({
      ...
      target_address: bs58.encode(randomBytes(32)),
      target_chain: 7,
      token_address: bs58.encode(randomBytes(32)),
      token_chain: 9,
      from_owner: bs58.encode(randomBytes(32)),
      fee: new BN(2),
    });

    console.log(JSON.stringify(wh, null, 4));

    const whBits = borshSerialize(wh);
