...
struct PulsexExactInActionParams {
  Asset toAsset;
  PulsexRoute[] routes;
  int256 minExchangeRate;
}

contract PulsexExactInAction is IWorkflowStep {
  event PulsexExactInActionEvent(address from, address to, uint256 amount, bool useNative);
  ...
  struct Locals {
    PulsexExactInActionParams args;
  ...
  function execute(AssetAmount[] calldata inputAssetAmounts, bytes calldata argData) public payable returns (WorkflowStepResult memory) {
    console.log('entering pulsex exact in action');
    // validate
    require(inputAssetAmounts.length == 1, 'there must be exactly 1 input asset');

    Locals memory locals;

    locals.args = abi.decode(argData, (PulsexExactInActionParams));
    console.log('decoded');
    // same from/to fails in the pulsex sdk
    // // check for no-op
    // if (
    ...
    for (uint256 i = 0; i < locals.args.routes.length; i++) {
      ...
      PulsexRoute memory route = locals.args.routes[i];
      ...
      console.log('calling pulsex for amount', amount);
  ...
  function logArgs(PulsexExactInActionParams memory args) internal view {
